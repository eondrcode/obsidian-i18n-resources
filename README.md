...
注册系统支持自动验证翻译仓库的合法性，检查必要的 `metadata.json` 或 `manifest.json` 文件格式，并自动提取翻译语言、插件列表等关键信息。管理员只需在 Issue 上添加标签即可完成审核，系统会自动处理后续的数据更新工作。
...
1. **准备翻译仓库**：确保您的翻译仓库包含有效的 `metadata.json` 或 `manifest.json` 文件，文件格式符合 Obsidian 插件翻译规范
...
**metadata.json（推荐）或 manifest.json**
...
1. **Prepare Your Repository**: Ensure your translation repository contains a valid `metadata.json` or `manifest.json` file that conforms to Obsidian plugin translation specifications
...
**metadata.json (Recommended) or manifest.json**
